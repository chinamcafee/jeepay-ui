<template>
  <div>
    ...
    <AppleIapProductForm ref="productFormRef" :app-id="appId" @saved="afterMutation" />
    <AppleIapProductImportDrawer ref="importDrawerRef" :app-id="appId" @imported="afterMutation" />
  </div>
</template>
...
async function load(page = 1) {
  ...
  vdata.error = ''
  try {
    const result = await listAppleIapProducts(props.appId, {
      pageNumber: page,
...
function disable(record) {
  $infoBox.confirmDanger(
    ...
    async () => {
      try {
        await disableAppleIapProduct(props.appId, record.productMappingId, record.rowVersion)
...
function remove(record) {
  $infoBox.confirmDanger(
    ...
    async () => {
      try {
        await deleteAppleIapProduct(props.appId, record.productMappingId, record.rowVersion)

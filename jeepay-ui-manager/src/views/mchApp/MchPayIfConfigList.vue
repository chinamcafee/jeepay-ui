...
async function reqCardListFunc() {
  ...
  if (appleRecord && $access('ENT_APPLE_IAP_CONFIG_VIEW')) {
    try {
      appleRecord.appleIapConfig = await getAppleIapConfig(vdata.appId, vdata.mchNo)
    } catch (error: any) {
...
function editPayIfConfigFunc(record) {
  ...
  if (record.subMchIsvConfig === 0) {
    ...
    appleIapPayConfig.value.show(vdata.appId, record, vdata.mchNo)

...
async function reloadLatest() {
  ...
  vdata.conflict = false
  try {
    const record = await getAppleIapProduct(props.mchNo, props.appId, vdata.productMappingId)
    applyRecord(record)
  } catch (error: any) {
...
async function executeSave() {
  ...
  try {
    if (vdata.productMappingId) {
      await updateAppleIapProduct(props.mchNo, props.appId, vdata.productMappingId, payload)
    } else {
      await createAppleIapProduct(props.mchNo, props.appId, payload)
    }

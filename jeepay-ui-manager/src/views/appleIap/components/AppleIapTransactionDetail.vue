...
async function load() {
  ...
  vdata.error = ''
  try {
    vdata.record = await getAppleIapTransaction(props.mchNo, props.appId, vdata.id)
  } catch (error: any) {
...
function openVerificationRetry() {
  actionModalRef.value.show({
    ...
    expectedState: 'DEAD',
    action: (data) =>
      retryAppleIapTransactionVerification(props.mchNo, props.appId, vdata.id, data),
  })
}
function openFinishRetry() {
  actionModalRef.value.show({
    ...
    expectedState: 'DEAD',
    action: (data) => retryAppleIapTransactionFinish(props.mchNo, props.appId, vdata.id, data),
  })
}
